...
def main():
	...
	num_train = 40
	num_test = 20
	red_dots = np.array([circle_cluster(10,10,5) for i in range(num_train//2)])
	blue_dots = np.array([circle_cluster(20,0,5) for i in range(num_train//2)])
	Xtrain = np.concatenate((red_dots,blue_dots))
	ytrain = np.array([0 for _ in range(num_train//2)]+[1 for _ in range(num_train//2)])
	
	red_dots_test = np.array([circle_cluster(10,10,5) for i in range(num_test//2)])
	blue_dots_test = np.array([circle_cluster(20,0,5) for i in range(num_test//2)])
